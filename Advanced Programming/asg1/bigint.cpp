...
bigint bigint::operator+ (const bigint& that) const {
  ...
  if(is_negative == that.is_negative)
      ...
      if(uvalue < that.uvalue) //uvalue<that.uvalue 
            //thus result has to be positive and is bigger number - smaller(negative number) 
          return {that.uvalue - uvalue, that.is_negative};
      else if (uvalue > that.uvalue)//uvalue>that.uvalue 
            //that.uvalue is negative so result is uvalue-that.uvalue. takes the sign of the bigger #
          return {uvalue - that.uvalue, is_negative};
    }
  //not same sign, and not bigger or smaller. thus same number but one is pos and one is neg
...
bigint bigint::operator- (const bigint& that) const {
  ...
  if(is_negative == that.is_negative){
      ...
      if(uvalue < that.uvalue)
          //since uvalue is smaller we can just subtract it from that.uvalue
          //take the opposite of the uvalue's sign
          return {that.uvalue - uvalue, !is_negative};
      else if (uvalue > that.uvalue)
          //since uvalue is bigger we can just subtract that.uvalue from it 
          //take the uvalue's sign because it was the bigger number
          return {uvalue - that.uvalue, is_negative};
      else
          //else they are the same sign and same number thus difference is 0
            return {ubigint {0}, false};
    }
  //
    return {uvalue + that.uvalue, is_negative};
...
bigint bigint::operator* (const bigint& that) const {
  if(is_negative == that.is_negative)
  {
    return {uvalue * that.uvalue, false}; //if the signs are the same then the result of the multiplication 
                                          //between both numbers will be pos
   
    
  }
   return {uvalue * that.uvalue, true};   //if the signs are different then the result of the multiplication 
...
bigint bigint::operator/ (const bigint& that) const {
  if(is_negative == that.is_negative)
  {
    return {uvalue / that.uvalue, false}; //if the signs are the same then the 
                                          //result of the division between both numbers will be pos
  }
    
  return {uvalue / that.uvalue, true}; //if the signs are different then the 
...
bigint bigint::operator% (const bigint& that) const {
  ...
  return {uvalue % that.uvalue, false}; //if the signs are the same then the result
                                          
}

bool bigint::operator== (const bigint& that) const {
  return is_negative == that.is_negative and uvalue == that.uvalue; //signs and numbers are same
}
